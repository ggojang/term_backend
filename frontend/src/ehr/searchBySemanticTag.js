...
/*
 * FSN과 Synonym 모두에서 q 를 찾기 때문에 결과에 concept id 중복 가능
*/

export default function SearchBySemanticTag(props) {
  ...
  const [semanticType, setSemanticType] = useState('Procedure');
  ...
  useEffect(() => {
    if (q.length > 0) {
      ...
      switch(semanticType) {
        case 'Procedure':
          opt = `match=PARTIAL&state=ACTIVE&q=${q}&semanticfilter=procedure&page=${page}&size=${size}`;
          break;
        case 'Disorder':
          opt = `match=PARTIAL&state=ACTIVE&q=${q}&semanticfilter=disorder&page=${page}&size=${size}`;
          break;
        case 'Clinical Finding':
          opt = `match=PARTIAL&state=ACTIVE&q=${q}&semanticfilter=finding&page=${page}&size=${size}`;
          break;
      }
  ...
  const handleSemanticTypeChange = (event) => {
  ...
  return (
    <>
      <Typography variant="h6">
        4.4.2* Constrain Search by Semantic Tag
      </Typography>
      <br />
