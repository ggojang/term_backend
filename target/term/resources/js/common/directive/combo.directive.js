(function () {
	...
	combo.$inject = ['$compile', '$timeout', '$document'];
	function combo($compile, $timeout, $document) {
		var iconArrowTop = 'glyphicon-triangle-top',
        iconArrowBottom = 'glyphicon-triangle-bottom',
        emptyMessage = 'selected...',
        template = '<div '
                + '  ng-keyup="comboVm.onKeyup($event)" '
                + '  ng-blur="comboVm.onBlur($event)" '
                + '  ng-dblclick="comboVm.onDblclick($event, true)" '
                + '  ng-style="comboVm.isEditMode() ? '
                + '              { padding: \'6px\' } : '
                + '              { padding: \'8px\' }   ">'
                + '  <span '
                + '    ng-if="comboVm.isEditMode()" '
                + '    class="combolist-arrow glyphicon" '
                + '    ng-class="comboVm.isLayerMode() ? '
                + '                \'' + iconArrowTop + '\' : '
                + '                \'' + iconArrowBottom + '\'"></span>'
                + '  <span '
                + '    class="combo-value" '
                + '    ng-class="comboVm.isEditMode() ? \'ellipsis-span\':\'\'" '
                + '    ng-bind-html="comboVm.isEditMode() && comboVm.model.name == null ? '
                + '                    \'' + emptyMessage + '\' : '
                + '                    (!comboVm.isEditMode() && comboVm.model.name == null ? \'&nbsp;\':comboVm.termBeautifier(comboVm.model.name))" '
                + '    ng-attr-conceptId={{comboVm.model.id}}></span>'
                + '</div>';
		...
		function link(scope, element, attributes, vm) {
			...
			// 편집모드 관련 메소드
		      ...
		      var linkFn,
		          ...
		          repeat = 'node in comboVm.tree | orderBy:\'term\'',
	...
	ComboController.$inject = ['$scope', 'apiservice', 'utilservice'];
	function ComboController($scope, apiservice, utilservice) {
		var vm = this;
	    ...
	    function _activate() {
	      ...
	      //apiservice.getChildren(vm.conceptId)
	    ...
	    function _getChildren(node) {
	      ...
	      apiservice.getChildren(node.conceptId)
